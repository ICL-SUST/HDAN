...
class FCN(nn.Module):
    ...
    def __init__(self, input_dim=2, growth_rate=16, block_config=(6, 12, 24, 16),
                 num_init_features=32, bn_size=4, drop_rate=0, num_classes=4):

        super(FCN, self).__init__()

        # First three convolutions
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv3d(input_dim, num_init_features, kernel_size=3, stride=1, padding=1, bias=False)),
...
if __name__ == '__main__':
    ...
    net = FCN(num_init_features=32, growth_rate=16, block_config=(4, 4, 4, 4), num_classes=4).to(device)
    input=torch.randn(1,2,32,64,64).to(device)

    output=net(input)
    print(output.shape)
    layer1=nn.Conv3d(2, 16, kernel_size=(3,3,3), stride=1, padding=0, bias=False).to(device)
    out=layer1(input)
    print(out.shape)
